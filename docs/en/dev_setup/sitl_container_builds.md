...
The interface-library pin includes the manual WaitSet ownership fixes proposed in [Auterion/px4-ros2-interface-lib#222](https://github.com/Auterion/px4-ros2-interface-lib/pull/222), which is still under upstream review.
...
Replace the temporary interface-library PR pin with its merged upstream commit once available.
...
The standalone `ghcr.io/px4/px4-dev-ros2:main-jazzy` image includes Jazzy, the Agent, PX4's source-build dependencies, pinned ROS sources and their dependencies.
It contains neither packaged PX4 firmware nor a compiled ROS workspace, and its entrypoint sources only the Jazzy underlay.
It is published independently of the SITL package workflow, so it does not wait for `.deb` builds.
The development publisher also supports manual builds; deployments must be explicitly enabled outside `main` pushes.

The `main-jazzy` tag tracks the toolchain for PX4 `main` development with ROS Jazzy, not bundled firmware.
...
Each run imports the pinned repositories directly with vcstool, without copying the image's precloned working trees or compiled workspaces.
...
The `ros_dev_container.yml` workflow publishes to GHCR on relevant `main` pushes or an explicitly enabled manual deployment.
Architecture tags use `sha-<full-PX4-commit>-jazzy-<architecture>`; the final `main-jazzy` and `sha-<full-PX4-commit>-jazzy` indexes preserve both architectures and their SBOMs.
Pull requests build the standalone target without publishing.
